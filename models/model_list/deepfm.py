...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope("output"):
                self.out = tf.add(tf.matmul(concat_input, self.weights["concat_projection"], name='Mul'),
                                  self.weights["concat_bias"], name='Add')
            if self.loss_type == "logloss":
                self.out = tf.nn.sigmoid(self.out, name='sigmoid_out')
    ...
    def get_batch(self, Xi, Xv, y, batch_size, index):
        ...
        #print('y:',[[y_] for y_ in y[start:end]])
    ...
    def predict(self, Xi, Xv):
        ...
        while len(Xi_batch) > 0:
            ...
            feed_dict = {self.feat_index: Xi_batch,
                         ...
                         self.label: np.array(y_batch).reshape([self.batch_size,1]),
